var procrastinator = (function() {
	...
	this.canRunBlocker = function(website)
	{
		return _enabled &&
			(
				...
				||
					(_timecodeControl === 'site' && website.timecode().isActive())
				||
					(_timecodeControl === 'global' && _timecodeGlobal.isActive())
			);
	}
	...
	function _loadState(callback)
	{
		...
		chrome.storage.sync.get(null, function(items) {
			...
			if (items.websites !== null && items.websites.length > 2) {
				try {
					_websites = JSON.parse(items.websites) || [];
					// correct the timecodes
	
					for(var i = 0, len = _websites.length; i < len; i++) {
						_websites[i] = new Website(_websites[i].pattern, _websites[i].timecode);
					}
